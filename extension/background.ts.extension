// Background service worker pour l'extension Etsmart
// Types locaux pour éviter les imports (service worker en script classique)
interface ShopData {
  shopUrl: string;
  shopName: string;
  salesCount?: number;
...
function getApiBaseUrl(messageApiUrl?: string): string {
  // Toujours utiliser l'URL publique, ignorer localhost
  return API_BASE_URL;
}

// Écouter les messages externes depuis le frontend web
chrome.runtime.onMessageExternal.addListener((message: ScrapeMessage, sender, sendResponse) => {
  try {
  ...
  try {
    if (message.type === 'OPEN_ANALYZING_PAGE' && message.niche) {
      // Vérifier d'abord si un onglet "analyse en cours" existe déjà
        chrome.storage.local.get(['analyzingTabId', 'apiBaseUrl'], async (result: { analyzingTabId?: number; apiBaseUrl?: string }) => {
    ...
    if (message.type === 'SCRAPED_DATA' && message.data) {
      ...
      chrome.storage.local.get(['currentNiche', 'currentSearchUrl'], (result: { currentNiche?: string; currentSearchUrl?: string }) => {
...
async function handleImport(searchUrl: string, niche: string): Promise<void> {
  try {
    ...
    await chrome.tabs.sendMessage(tab.id!, { type: 'SCRAPE_NOW' } as ScrapeMessage);
...
async function analyzeSingleShop(shopData: SingleShopData): Promise<void> {
  try {
    // Ouvrir la page "Analyse en cours" pour une boutique
    let resultsTabId: number | undefined;
    
    // Vérifier si un onglet existe déjà
    const storage = await chrome.storage.local.get(['analyzingTabId', 'apiBaseUrl']) as { analyzingTabId?: number; apiBaseUrl?: string };
...
async function sendToAPI(niche: string, shops: ShopData[], searchUrl?: string): Promise<void> {
  try {
    // Récupérer l'URL de l'API depuis le storage (ignorer localhost)
    const storage = await chrome.storage.local.get(['analyzingTabId', 'apiBaseUrl']) as { analyzingTabId?: number; apiBaseUrl?: string };
    const apiBaseUrl = (storage.apiBaseUrl && typeof storage.apiBaseUrl === 'string' && !storage.apiBaseUrl.includes('localhost')) ? storage.apiBaseUrl : API_BASE_URL;
