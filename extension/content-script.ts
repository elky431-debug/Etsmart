// Content script pour scraper les boutiques Etsy
// Types locaux pour éviter les imports (script classique)
interface ShopData {
  shopUrl: string;
  shopName: string;
  salesCount?: number;
...
interface ScrapeMessage {
  type: 'SCRAPE_NOW' | 'SCRAPED_DATA' | 'START_IMPORT' | 'OPEN_ANALYZING_PAGE' | 'ANALYZE_SHOP';
  data?: ShopData[];
  shopData?: SingleShopData;
  niche?: string;
  searchUrl?: string;
  shopUrl?: string;
}

interface SingleShopData {
  shopUrl: string;
  shopName: string;
  salesCount?: number;
  rating?: number;
...
let lastKnownNiche: string | null = null;

// Système de logs persistants
function logPersistent(message: string, level: 'log' | 'warn' | 'error' = 'log') {
...
function createImportButton() {
  ...
  importButton.addEventListener('mouseenter', () => {
    if (importButton) importButton.style.transform = 'scale(1.05)';
  });
  
  importButton.addEventListener('mouseleave', () => {
    if (importButton) importButton.style.transform = 'scale(1)';
  });
  ...
  importButton.addEventListener('click', async (e) => {
    ...
    if (isShop) {
      ...
      try {
        const response = await new Promise<{ success?: boolean; tabId?: number } | undefined>((resolve) => {
          chrome.runtime.sendMessage({
            type: 'OPEN_ANALYZING_PAGE',
            niche
          } as ScrapeMessage, (resp) => {
...
function extractShopData(shopElement: Element): ShopData | null {
  try {
    ...
    let shopLink = listingCard.querySelector('a[href*="/shop/"]') as HTMLAnchorElement | null;
    ...
    if (!shopName) {
      const dataShopName = (listingCard as HTMLElement).getAttribute('data-shop-name');
      if (dataShopName) {
...
async function analyzeSingleShop(): Promise<void> {
  try {
    ...
    for (const card of Array.from(listingCards).slice(0, 50)) { // Limiter à 50 listings
      try {
        const listingLink = card.querySelector('a[href*="/listing/"]') as HTMLAnchorElement || 
                           (card as HTMLAnchorElement);
        const listingUrl = listingLink?.href?.split('?')[0];
        
        if (!listingUrl || seenListingUrls.has(listingUrl)) continue;
...
async function scrapeShops(): Promise<void> {
  try {
...
function displayPersistentLogs() {
  try {
    ...
    if (logs.length > 0) {
      ...
      console.log('%c═══════════════════════════════════════════════════════════════', 'color: #00d4ff; font-size: 14px; font-weight: bold;');
      logs.forEach((log: { timestamp: string; message: string; level: string }) => {
        const color = log.level === 'error' ? '#ff4444' : log.level === 'warn' ? '#ffaa00' : '#00d4ff';
        console[log.level as 'log' | 'warn' | 'error'](`%c[ETSMART] ${log.message}`, `color: ${color}; font-weight: bold;`);
      });
      console.log('%c═══════════════════════════════════════════════════════════════', 'color: #00d4ff; font-size: 14px; font-weight: bold;');
    } else {
