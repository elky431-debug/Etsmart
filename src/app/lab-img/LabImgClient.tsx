...
const compressImage = (file: File): Promise<string> =>
  new Promise((resolve, reject) => {
    if (typeof window === 'undefined' || typeof document === 'undefined') {
      reject(new Error('Compression disponible uniquement dans le navigateur'));
      return;
    }
    const Ctor = typeof Image !== 'undefined' ? Image : null;
    if (!Ctor) {
      reject(new Error('API Image indisponible'));
      return;
    }
...
export default function LabImgClient() {
  ...
  const generate = async () => {
    if (!sourceBase64) {
      setError("Choisir une image d'abord");
      return;
    }
    if (!secretKey) {
      setError('Clé secrète manquante dans l\'URL (?key=...)');
      return;
    }
    stopPolling();
    ...
    <div className="min-h-screen bg-black text-white p-6 font-mono">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="space-y-6">
          ...
          >
            {isProcessing ? (
              <>
                ...
                {status === 'polling' ? `Polling... (task: ${taskId?.slice(0, 12)}...)` : 'Génération...'}
              </>
            ) : (
              <>
          ...
          {error && (
            <div className="p-4 bg-red-900/40 border border-red-500/50 rounded-xl text-red-300 text-sm">❌ {error}</div>
          )}
          ...
          {log.length > 0 && (
            <div>
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-bold text-zinc-500">LOG</span>
                <button type="button" onClick={() => setLog([])} className="text-xs text-zinc-600 hover:text-zinc-400">
                  Effacer
                </button>
              </div>
