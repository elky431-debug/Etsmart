  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (errorParam === 'oauth_error') {
        setError(messageParam || 'Erreur lors de la connexion avec Google. Veuillez réessayer.');
        // Nettoyer l'URL
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (errorMessage.includes('provider is not enabled') || errorMessage.includes('Unsupported provider')) {
        setError('Le fournisseur Google n\'est pas activé dans Supabase. Vérifiez la configuration dans Supabase → Authentication → Providers.');
      } else if (errorMessage.includes('redirect_uri_mismatch')) {
        setError('URL de redirection incorrecte. Vérifiez la configuration dans Google Cloud Console et Supabase.');
      } else {
      ...
      >
        ...
        >
          ...
          <motion.button
            ...
            className="w-full flex items-center justify-center gap-3 py-3.5 bg-white/5 border-2 border-white/10 text-white font-semibold rounded-lg hover:border-white/20 hover:bg-white/10 transition-all shadow-sm disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer mb-6"
          >
            {isGoogleLoading ? (
          ...
          <div className="relative mb-6">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="px-2 bg-white/5 text-white/50">ou</span>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <label className="block text-sm font-semibold text-white/90 mb-2">Mot de passe</label>
              <div className="relative group">
                <div className="absolute left-4 top-1/2 -translate-y-1/2 text-white/60 group-focus-within:text-[#00d4ff] transition-colors">
                  <Lock size={18} />
                </div>
            ...
            <div className="flex items-center justify-between text-sm">
              <label className="flex items-center gap-2 cursor-pointer group">
                <input 
                  type="checkbox" 
                  className="w-4 h-4 rounded border-white/30 bg-white/5 text-[#00d4ff] focus:ring-[#00d4ff] focus:ring-2 cursor-pointer" 
                />
            ...
            <Link href="/forgot-password">
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                className="w-full flex items-center justify-center gap-2 py-3 bg-white/5 border-2 border-white/10 text-white/80 font-semibold rounded-lg hover:border-[#00d4ff]/50 hover:text-[#00d4ff] hover:bg-white/10 transition-all cursor-pointer"
              >
                <HelpCircle size={18} />
                Mot de passe oublié ?
              </motion.button>
        ...
        <p className="text-center text-white/50 text-sm mt-8">
          ...
          <a href="#" className="text-white/70 hover:text-[#00d4ff] transition-colors">conditions d'utilisation</a>
        </p>
      </motion.div>
    </div>
  );
